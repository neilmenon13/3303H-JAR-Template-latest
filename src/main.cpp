...
void skills()
{
  ...
  RightDriveSmart.setVelocity(100, percent);
  chassis.turn_to_angle(180);
  chassis.drive_distance(13.887);
  chassis.turn_to_angle(-90);
  chassis.drive_distance(80);
  chassis.turn_to_angle(0);
  chassis.drive_distance(15.887);
  chassis.turn_to_angle(-90);
  LeftDriveSmart.setVelocity(67, percent);
  RightDriveSmart.setVelocity(67, percent);
  chassis.set_drive_exit_conditions(1.5, 300, 500);
  ...
  // chassis.drive_distance(-13.887);
  // chassis.turn_to_angle(-90);
  // chassis.drive_distance(85.159);
  // chassis.turn_to_angle(180);
  // chassis.drive_distance(-11.887);
  // chassis.turn_to_angle(-90);
  // LeftDriveSmart.setVelocity(67,percent);
  // RightDriveSmart.setVelocity(67,percent);
  // chassis.set_drive_exit_conditions(1.5, 300, 500);
  // chassis.drive_distance(-11.16);
  // outtakeMotor.spin(forward, 100, percent);
  // intakeMotor.spin(forward, 100, percent);
  // wait(2, seconds);
  // outtakeMotor.stop();
...
void profiling()
{
  ...
  if (!dataFile.is_open())
  {
    Brain.Screen.printAt(5, 200, "Profiling: cannot open data.txt");
    return;
  }

  dataFile.setf(std::ios::fixed);
  dataFile.precision(2);
  dataFile << "timestamp,left1,left2,left3,right1,right2,right3\n";

  int flushCounter = 0;
  while (true)
  {
    double timestamp = Brain.Timer.value();
    double left1 = leftMotor1.velocity(rpm);
    double left2 = leftMotor2.velocity(rpm);
    double left3 = leftMotor3.velocity(rpm);
    double right1 = rightMotor1.velocity(rpm);
    double right2 = rightMotor2.velocity(rpm);
    double right3 = rightMotor3.velocity(rpm);

    dataFile << timestamp << "," << left1 << "," << left2 << "," << left3 << ","
...
void usercontrol(void)
{
  ...
  while (1)
  {
    ...
    {
      descorer_status = !descorer_status;
      waitUntil(!Controller.ButtonY.pressing());
    }
    descorer.set(!descorer_status);
  }
}
